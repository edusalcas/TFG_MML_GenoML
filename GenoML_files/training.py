...
class train:
    def __init__(self, df, run_prefix, algs_name=None, seed=42):
        ...
        # Split the data
        X_train, X_test, y_train, y_test = model_selection.train_test_split(X, y, test_size=0.3, random_state=seed)  # 70:30
    ...
    def getAlgorithmFromName(self, alg_name):
        ...
        if alg_name == 'LogisticRegression':
            ...
            sys.exit('Algorithm name ' + alg_name + ' incorrect, please check it')
    ...
    def results(self, metric_max):
        ...
        if(metric_max == "Balanced_Accuracy"):
            ...
            trained_mean_out = self.run_prefix + '.trainedMean.txt'

            # Save trained_mean
            file = open(trained_mean_out, "w+")
            file.write(str(self.log_table.Balanced_Accuracy_Percent.max()))
            file.close()
        ...
        if not isinstance(best_algo, str):
            best_algo = best_algo[0]
        self.best_algo = best_algo
